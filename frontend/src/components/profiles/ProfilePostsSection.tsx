...
export default function ProfilePostsSection({ profileId, refetchProfile }: { profileId: string, refetchProfile: () => void }) {

	const fetchPosts = useCallback(async ({ pageParam }: { pageParam?: { id: string, createdAt: string } }): Promise<APIResponse> => {
		return await fetchData('/api/profiles/get-posts', {
			profileId,
			cursor: pageParam
		});
	}, [profileId]);
