...
export default function ProfileFollowersSection({ profileId, refetchProfile, isOwnProfile }: { profileId: string, refetchProfile: () => void, isOwnProfile: boolean }) {

	const fetchProfiles = useCallback(async ({ pageParam }: { pageParam?: { id: string, followedAt: string } }): Promise<APIResponse> => {
		return await fetchData('/api/profiles/get-followers', {
			profileId,
			cursor: pageParam
		});
	}, [profileId]);
